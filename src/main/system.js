...
function clearSparkleCache() {
  try {
    ...
    const logsPath = path.join(appDataPath, "maxify", "logs")
    ...
    if (fs.existsSync(scriptsPath)) {
      ...
      scriptsCleared = true
      console.log("Arquivos do diretório de scripts do Maxify limpos com sucesso.")
    } else {
      console.warn("O diretório de scripts do Maxify não existe.")
      errors.push("O diretório de scripts não existe.")
    }
    ...
    if (fs.existsSync(logsPath)) {
      ...
      for (const file of logFiles) {
        ...
        try {
          ...
          errors.push(`Falha ao excluir o arquivo de log:${file} - ${err.message}`)
        }
      }
      logsCleared = true
      console.log("Arquivos do diretório de logs do Maxify limpos com sucesso.")
    } else {
      console.warn("O diretório de logs do Maxify não existe.")
      errors.push("O diretório de logs não existe.")
    }
    ...
    console.error("Falha ao limpar scripts ou diretório de logs do Maxify:", error)
    return { success: false, error: error.message }
  }
}
...
function openLogFolder() {
  ...
  if (fs.existsSync(logPath)) {
    ...
    console.warn("O diretório de logs do Maxify não existe.")
    return { success: false, error: "O diretório de logs não existe.w" }
  }
}
...
function Show-InstallerGUI {
    ...
    $form.Text = "Maxify: Winget Installer"
    $form.Size = New-Object System.Drawing.Size(600,400)
    $form.StartPosition = "CenterScreen"

    $label = New-Object System.Windows.Forms.Label
    $label.Text = "Welcome! Maxify needs Winget to install apps."
    $label.AutoSize = $true
    ...
    [void]$powershell.AddScript({
        ...
        try {
            ...
            if ($TestMode -or -not $wingetInstalled) {
                $result.Messages += "Winget not found. Installing for Maxify..."
                ...
                $result.Messages += "Winget is already installed. Maxify is ready to install apps!"
                $result.Success = $true
            }
        } catch {
...
if ($TestMode -or -not (Check-Winget)) {
    ...
    Write-Output "Winget is already installed. Maxify can install apps!"
}
`
...
ipcMain.handle("check-winget", async () => {
  try {
    ...
    console.error("Falha ao verificar o Winget:", error)
