...
function isAuthValid() {
  if (authSession.mode === "free") {
    return Boolean(
      authSession.logged === true &&
      authSession.token === "FREE_LOCAL_SESSION" &&
...
async function authorizePowerShellExecution(payload, action = "run-powershell") {
  ...
  // Se estiver logado no modo Free, libera apenas funções Free
  if (isFreeSession()) {
    if (isFreeAllowedPayload) {
      return true
    }
  ...
  // Correção: se o renderer marcou Free, mas o main perdeu a sessão,
  // ainda libera SOMENTE payloads seguros/free.
  if (!isAuthValid()) {
    if (isFreeAllowedPayload) {
      console.warn("[Maxify Free]: sessão main ausente, mas payload Free seguro liberado.")
      return true
    }
...
ipcMain.handle("auth:login-free", async () => {
  // Inicialização é PRO: garante que o modo Free não tenha handlers de startup.
  unregisterStartupManagerHandlers()
...
ipcMain.handle("auth:login-saved", async () => {
  ...
  return {
    success: false,
    error: "Login automático desativado. Digite sua key novamente.",
  }
})
...
ipcMain.handle("invoke", async (event, data) => {
  ...
  try {
    switch (channel) {
      ...
      case "run-powershell":
        await authorizePowerShellExecution(payload)
