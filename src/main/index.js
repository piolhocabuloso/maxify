...
const FEEDBACK_WEBHOOK_URL =
  ...
  "https://discord.com/api/webhooks/1506496209097982002/Wb_Py1UrxuNdvMLNYyOC33tRROdDQIStmSXwUB4LencSOWpXfVcTqcxbAxMWq9QrvG7-"
...
function getFeedbackColor(category = "") {
  const type = String(category).toLowerCase()

  if (type.includes("bug")) return 0x05F804
  if (type.includes("suggestion") || type.includes("sugest")) return 0x05F804
  if (type.includes("feedback")) return 0x05F804

  return 0x05F804
}

function getFeedbackEmoji(category = "") {
  const type = String(category).toLowerCase()

  if (type.includes("bug")) return "<:icons8termsandconditions100:1335352538719060059>"
  if (type.includes("suggestion") || type.includes("sugest")) return "<:icons8termsandconditions100:1335352538719060059>"
  if (type.includes("feedback")) return "<:icons8termsandconditions100:1335352538719060059>"

  return "<:icons8termsandconditions100:1335352538719060059>"
}

function formatFeedbackUser(payloadUser = {}) {
  ...
  const user = authSession?.user || payloadUser || currentDiscord || {}
  const discord = currentDiscord || payloadUser?.discord || payloadUser || {}
...
async function sendDiscordWebhook(webhookUrl, body) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(text || `Discord retornou status ${response.status}`)
    }
...
ipcMain.handle("feedback:send", async (_, payload = {}) => {
  ...
  if (now - lastFeedbackAt < 8000) {
    return {
      success: false,
      message: "Aguarde alguns segundos para enviar outro relato.",
    }
  }

  const webhookUrl = String(FEEDBACK_WEBHOOK_URL || "").trim()

  if (
    !webhookUrl ||
    webhookUrl.includes("COLOQUE_SEU_WEBHOOK") ||
  ...
  const categoryLabel = safeFeedbackText(
    payload.categoryLabel || payload.category || "Feedback",
    40
  )

  const message = safeFeedbackText(payload.message, 900)
    ...
    embeds: [
      {
        ...
        fields: [
          ...
          {
            name: "<:icons8documentosdeidentificaover:1333014529528954890> Discord ID",
            value: feedbackUser.discordId,
            inline: true,
          },
          {
            name: "<:icons8touchpad100:1337855935120539791> Plano",
            value: feedbackUser.plan,
            inline: true,
          },
          {
          ...
          {
            name: "<:icons8mensagem100:1332965074066870363> HWID",
            value: feedbackUser.hwid,
            inline: true,
          },
          {
            name: "<:icons8comunicao100:1338904005081825432> Mensagem",
            value: message,
            inline: false,
          },
  ...
  try {
    await sendDiscordWebhook(webhookUrl, body)
