...
const ensureScriptsAvailable = async () => {
  ...
  try {
    ...
    console.log("Copiando scripts DNS para o diretório de dados do usuário...")
      ...
      try {
        ...
        for (const file of files) {
          ...
          try {
            ...
            console.log(`Copiado ${file} para os dados do usuário`)
          } catch (copyError) {
            console.warn(`Falha ao copiar ${file}:`, copyError.message)
          }
        }
        break
      } catch (error) {}
...
const getScriptPath = async (scriptName) => {
  ...
  for (const scriptPath of possiblePaths) {
    try {
      ...
      console.log(`Script DNS encontrado em: ${scriptPath}`)
      return scriptPath
    } catch (error) {}
  }

  throw new Error(
    `Script DNS não encontrado: ${scriptName}. Caminhos pesquisados: ${possiblePaths.join(", ")}`,
  )
}
