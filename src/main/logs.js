...
const WEBHOOK_URL = process.env.MAXIFY_LOG_WEBHOOK || "https://discord.com/api/webhooks/1408882704677732393/YViaLAncf41tvjBzE8e0hQJfx1FWVcX0bopw55vrR6A8wyO2HygxyCwH3hiVQCLd7lZi";
...
class LogsManager {
    ...
    setupDiscordListener() {
        ipcMain.on('discord-user-updated', (event, userInfo) => {
            this.discordUser = userInfo || null;
        });
    ...
    async collectSystemInfo() {
        try {
            ...
            const hwidSeed = [os.hostname(), os.platform(), os.arch(), os.totalmem(), os.cpus()?.[0]?.model].join("|");
            const hwidHash = crypto.createHash("sha256").update(hwidSeed).digest("hex");
        ...
        try {
            ...
            const embed = {
                ...
                fields: [
                    ...
                    { name: '<:icons8passfail100:1337860467426594927> HWID', value: `${info.hwidHash}`, inline: true },
                ],
