...
export default function Conta() {
    ...
    const logout = async () => {
        try {
            console.log("[Conta] Iniciando logout...")

            try {
                if (window.electron?.auth?.logout) {
                    console.log("[Conta] Chamando auth.logout...")
                    await window.electron.auth.logout()
                } else {
                    console.warn("[Conta] window.electron.auth.logout não existe")
                }
            } catch (err) {
            ...
            try {
                if (window.electron?.auth?.clearSavedKey) {
                    console.log("[Conta] Chamando auth.clearSavedKey...")
                    await window.electron.auth.clearSavedKey()
                } else {
                    console.warn("[Conta] window.electron.auth.clearSavedKey não existe")
                }
            } catch (err) {
            ...
            setTimeout(() => {
                console.log("[Conta] Enviando logout para o App...")
