...
const tabs = {
  ...
  apps: { label: "Input Lag", path: "/apps" },
...
function NavButton({ id, tab, active, expanded, onClick, locked = false }) {
    ...
    >
        ...
        {!expanded && locked && (
          <span
            className="
              ...
              bg-[#111827]
              text-yellow-300
              shadow-[0_0_8px_rgba(250,204,21,0.35)]
            "
          >
            <LockKeyhole size={7.5} strokeWidth={2.4} />
          </span>
        )}
      </motion.div>
      ...
      {expanded && locked && (
        <div className="relative z-10 ml-auto flex items-center pr-2">
          <span className="inline-flex items-center gap-1 rounded-full border border-yellow-400/20 bg-yellow-400/10 px-2 py-0.5 text-[10px] font-semibold text-yellow-300">
            <Crown size={11} /> PRO
          </span>
        </div>
      )}
      ...
      <NavTooltip
        label={locked ? `${tab.label} • Premium` : tab.label}
        expanded={expanded}
      />
    </motion.button>
  )
...
export default function Nav({ expanded, setExpanded, accessMode = "premium" }) {
  ...
  useEffect(() => {
    ...
    async function loadDiscordUser() {
      try {
        ...
        const user = result?.user || await invoke({ channel: "get-discord-user" }).catch(() => null)
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {mainTabs.map(([id, tab]) => {
            const locked = accessMode === "free" && !canAccessRoute(tab.path)
